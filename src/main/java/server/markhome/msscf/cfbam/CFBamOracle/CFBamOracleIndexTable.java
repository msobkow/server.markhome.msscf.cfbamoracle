// Description: Java 11 Oracle Jdbc DbIO implementation for Index.
...
/*
 *	CFBamOracleIndexTable Oracle Jdbc DbIO implementation
 *	for Index.
 */
	...
	implements ICFBamIndexTable
{
		...
		long TableId )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByIdxTableIdx = null;
		List<CFBamIndexBuff> buffList = new LinkedList<CFBamIndexBuff>();
		try {
			stmtReadBuffByIdxTableIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_idxdefbyidxtableidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByIdxTableIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByIdxTableIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByIdxTableIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamIndexBuff buff = unpackIndexResultSetToBuff( resultSet );
		...
		long argTableId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_idxdefbyidxtableidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByIdxTableIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
