// Description: Java 11 Oracle Jdbc DbIO implementation for DelSubDep1.
...
/*
 *	CFBamOracleDelSubDep1Table Oracle Jdbc DbIO implementation
 *	for DelSubDep1.
 */
	...
	implements ICFBamDelSubDep1Table
{
		...
		long RelationId )
	{
		...
		List<CFBamDelSubDep1Buff> buffList = new LinkedList<CFBamDelSubDep1Buff>();
		try {
			stmtReadBuffByDelDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_delsubdep1bydeldepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long DelTopDepId )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByDelTopDepIdx = null;
		List<CFBamDelSubDep1Buff> buffList = new LinkedList<CFBamDelSubDep1Buff>();
		try {
			stmtReadBuffByDelTopDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_delsubdep1bydeltopdepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByDelTopDepIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByDelTopDepIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByDelTopDepIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamDelSubDep1Buff buff = unpackDelSubDep1ResultSetToBuff( resultSet );
		...
		long argDelTopDepId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_delsubdep1bydeltopdepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByDelTopDepIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_delsubdep1bydeldepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
