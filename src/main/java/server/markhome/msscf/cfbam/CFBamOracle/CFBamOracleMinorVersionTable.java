// Description: Java 11 Oracle Jdbc DbIO implementation for MinorVersion.
...
/*
 *	CFBamOracleMinorVersionTable Oracle Jdbc DbIO implementation
 *	for MinorVersion.
 */
	...
	implements ICFBamMinorVersionTable
{
		...
		CFIntMinorVersionBuff Buff )
	{
		...
		try {
			...
			stmtCreateByPKey = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".crt_mnvrdef( ?, ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			...
			if( resultSet == null ) {
				throw new CFLibRuntimeException( getClass(),
					S_ProcName,
					"crt_mnvrdef() did not return a result set" );
			}
			try {
				if( resultSet.next() ) {
				...
				throw new CFLibRuntimeException( getClass(),
					S_ProcName,
					"crt_mnvrdef() did not return a valid result set" );
			}
		}
		catch( SQLException e ) {
	...
	public String getSqlSelectMinorVersionDistinctClassCode() {
		if( S_sqlSelectMinorVersionDistinctClassCode == null ) {
			S_sqlSelectMinorVersionDistinctClassCode =
				...
				+	"FROM " + schema.getLowerDbSchemaName() + ".mnvrdef a104 ";
		}
		...
		CFIntMinorVersionPKey PKey )
	{
		...
		try {
			...
			stmtReadBuffByPKey = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_mnvrdef( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		CFIntMinorVersionPKey PKey )
	{
		...
		try {
			...
			stmtLockBuffByPKey = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".lck_mnvrdef( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
	...
	public CFIntMinorVersionBuff[] readAllBuff( CFSecAuthorization Authorization ) {
		...
		try {
			CFIntMinorVersionBuff buff = null;
			List<CFIntMinorVersionBuff> buffList = new LinkedList<CFIntMinorVersionBuff>();
			stmtReadAllBuff = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_mnvrdefall( ?, ?, ?, ?, ?, ? ) ); end;" );
			int argIdx = 1;
		...
		long Id )
	{
		...
		try {
			stmtReadBuffByIdIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_mnvrdefbyididx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long TenantId )
	{
		...
		List<CFIntMinorVersionBuff> buffList = new LinkedList<CFIntMinorVersionBuff>();
		try {
			stmtReadBuffByTenantIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_mnvrdefbytenantidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long MajorVersionId )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByMajorVerIdx = null;
		List<CFIntMinorVersionBuff> buffList = new LinkedList<CFIntMinorVersionBuff>();
		try {
			stmtReadBuffByMajorVerIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_mnvrdefbymajorveridx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByMajorVerIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByMajorVerIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByMajorVerIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFIntMinorVersionBuff buff = unpackMinorVersionResultSetToBuff( resultSet );
		...
		String Name )
	{
		...
		try {
			stmtReadBuffByNameIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_mnvrdefbynameidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		CFIntMinorVersionBuff Buff )
	{
		...
		try {			long TenantId = Buff.getRequiredTenantId();
			...
			stmtUpdateByPKey = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".upd_mnvrdef( ?, ?, ?, ?, ?, ?, ?" + ", "
					+	"?" + ", "
					+	"?" + ", "
					+	"?" + ", "
					+	"?" + ", "
			...
			if( resultSet != null ) {
				...
				Buff.setRequiredRevision( updatedBuff.getRequiredRevision() );
					...
					throw new CFLibRuntimeException( getClass(),
						S_ProcName,
						"upd_mnvrdef() did not return a valid result cursor" );
				}
				finally {
					if( resultSet != null ) {
						try {
				...
				throw new CFLibRuntimeException( getClass(),
					S_ProcName,
					"upd_mnvrdef() did not return a result cursor" );
			}
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
		...
		CFIntMinorVersionBuff Buff )
	{
		...
		try {
			long TenantId = Buff.getRequiredTenantId();
			long Id = Buff.getRequiredId();
			stmtDeleteByPKey = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".dl_mnvrdef( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long argId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_mnvrdefbyididx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
		...
		long argTenantId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_mnvrdefbytenantidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + " ); end";
		...
		long argMajorVersionId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_mnvrdefbymajorveridx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByMajorVerIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
		...
		String argName )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_mnvrdefbynameidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
