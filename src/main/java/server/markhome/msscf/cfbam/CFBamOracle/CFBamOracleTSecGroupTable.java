// Description: Java 11 Oracle Jdbc DbIO implementation for TSecGroup.
...
/*
 *	CFBamOracleTSecGroupTable Oracle Jdbc DbIO implementation
 *	for TSecGroup.
 */
	...
	implements ICFBamTSecGroupTable
{
	...
	public String getSqlSelectTSecGroupDistinctClassCode() {
		if( S_sqlSelectTSecGroupDistinctClassCode == null ) {
			S_sqlSelectTSecGroupDistinctClassCode =
				...
				+	"FROM " + schema.getLowerDbSchemaName() + ".TSecGrp a016 ";
		}
		...
		boolean IsVisible )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByTenantVisIdx = null;
		List<CFSecTSecGroupBuff> buffList = new LinkedList<CFSecTSecGroupBuff>();
		try {
			stmtReadBuffByTenantVisIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_tsecgrpbytenantvisidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByTenantVisIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByTenantVisIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByTenantVisIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFSecTSecGroupBuff buff = unpackTSecGroupResultSetToBuff( resultSet );
		...
		boolean argIsVisible )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_tsecgrpbytenantvisidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByTenantVisIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
