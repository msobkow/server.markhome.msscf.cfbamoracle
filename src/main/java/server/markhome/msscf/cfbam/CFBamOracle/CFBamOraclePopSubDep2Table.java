// Description: Java 11 Oracle Jdbc DbIO implementation for PopSubDep2.
...
/*
 *	CFBamOraclePopSubDep2Table Oracle Jdbc DbIO implementation
 *	for PopSubDep2.
 */
	...
	implements ICFBamPopSubDep2Table
{
		...
		long PopSubDep1Id )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByPopSubDep1Idx = null;
		List<CFBamPopSubDep2Buff> buffList = new LinkedList<CFBamPopSubDep2Buff>();
		try {
			stmtReadBuffByPopSubDep1Idx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_popsubdep2bypopsubdep1idx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByPopSubDep1Idx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByPopSubDep1Idx.execute();
			resultSet = (ResultSet)stmtReadBuffByPopSubDep1Idx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamPopSubDep2Buff buff = unpackPopSubDep2ResultSetToBuff( resultSet );
		...
		long argPopSubDep1Id )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_popsubdep2bypopsubdep1idx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByPopSubDep1Idx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
