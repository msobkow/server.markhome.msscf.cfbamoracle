// Description: Java 11 Oracle Jdbc DbIO implementation for DelSubDep2.
...
/*
 *	CFBamOracleDelSubDep2Table Oracle Jdbc DbIO implementation
 *	for DelSubDep2.
 */
	...
	implements ICFBamDelSubDep2Table
{
		...
		long RelationId )
	{
		...
		List<CFBamDelSubDep2Buff> buffList = new LinkedList<CFBamDelSubDep2Buff>();
		try {
			stmtReadBuffByDelDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_delsubdep2bydeldepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_delsubdep2bydeldepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
