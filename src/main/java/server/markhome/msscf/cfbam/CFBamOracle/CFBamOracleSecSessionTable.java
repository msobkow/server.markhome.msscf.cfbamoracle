...
/*
 *	CFBamOracleSecSessionTable Oracle Jdbc DbIO implementation
 *	for SecSession.
 */
	...
	implements ICFBamSecSessionTable
{
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFBamOracleSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		...
		UUID priorSecSessionId )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtPageBuffByFinishIdx.setString( argIdx++, CFBamOracleSchema.getTimestampString( Finish ) );
			}
			else {
