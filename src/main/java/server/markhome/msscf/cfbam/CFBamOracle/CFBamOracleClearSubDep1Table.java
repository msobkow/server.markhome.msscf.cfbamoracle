// Description: Java 11 Oracle Jdbc DbIO implementation for ClearSubDep1.
...
/*
 *	CFBamOracleClearSubDep1Table Oracle Jdbc DbIO implementation
 *	for ClearSubDep1.
 */
	...
	implements ICFBamClearSubDep1Table
{
		...
		long RelationId )
	{
		...
		List<CFBamClearSubDep1Buff> buffList = new LinkedList<CFBamClearSubDep1Buff>();
		try {
			stmtReadBuffByClearDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep1bycleardepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		Long DefSchemaId )
	{
		...
		List<CFBamClearSubDep1Buff> buffList = new LinkedList<CFBamClearSubDep1Buff>();
		try {
			stmtReadBuffByDefSchemaIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep1bydefschemaidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long ClearTopDepId )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByClearTopDepIdx = null;
		List<CFBamClearSubDep1Buff> buffList = new LinkedList<CFBamClearSubDep1Buff>();
		try {
			stmtReadBuffByClearTopDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep1bycleartopdepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByClearTopDepIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByClearTopDepIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByClearTopDepIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamClearSubDep1Buff buff = unpackClearSubDep1ResultSetToBuff( resultSet );
		...
		long argClearTopDepId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep1bycleartopdepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByClearTopDepIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep1bycleardepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
		...
		Long argDefSchemaId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep1bydefschemaidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
