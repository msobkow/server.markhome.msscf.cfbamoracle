// Description: Java 11 Oracle Jdbc DbIO implementation for Chain.
...
/*
 *	CFBamOracleChainTable Oracle Jdbc DbIO implementation
 *	for Chain.
 */
	...
	implements ICFBamChainTable
{
	...
	public String getSqlSelectChainDistinctClassCode() {
		if( S_sqlSelectChainDistinctClassCode == null ) {
			S_sqlSelectChainDistinctClassCode =
				...
				+	"FROM " + schema.getLowerDbSchemaName() + ".chain_def a80f ";
		}
		...
		long TableId )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByChainTableIdx = null;
		List<CFBamChainBuff> buffList = new LinkedList<CFBamChainBuff>();
		try {
			stmtReadBuffByChainTableIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_chain_defbychaintableidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByChainTableIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByChainTableIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByChainTableIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamChainBuff buff = unpackChainResultSetToBuff( resultSet );
		...
		long argTableId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_chain_defbychaintableidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByChainTableIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
