// Description: Java 11 Oracle Jdbc DbIO implementation for ClearSubDep2.
...
/*
 *	CFBamOracleClearSubDep2Table Oracle Jdbc DbIO implementation
 *	for ClearSubDep2.
 */
	...
	implements ICFBamClearSubDep2Table
{
		...
		long RelationId )
	{
		...
		List<CFBamClearSubDep2Buff> buffList = new LinkedList<CFBamClearSubDep2Buff>();
		try {
			stmtReadBuffByClearDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep2bycleardepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		Long DefSchemaId )
	{
		...
		List<CFBamClearSubDep2Buff> buffList = new LinkedList<CFBamClearSubDep2Buff>();
		try {
			stmtReadBuffByDefSchemaIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep2bydefschemaidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long ClearSubDep1Id )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByClearSubDep1Idx = null;
		List<CFBamClearSubDep2Buff> buffList = new LinkedList<CFBamClearSubDep2Buff>();
		try {
			stmtReadBuffByClearSubDep1Idx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep2byclearsubdep1idx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByClearSubDep1Idx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByClearSubDep1Idx.execute();
			resultSet = (ResultSet)stmtReadBuffByClearSubDep1Idx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamClearSubDep2Buff buff = unpackClearSubDep2ResultSetToBuff( resultSet );
		...
		long argClearSubDep1Id )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep2byclearsubdep1idx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByClearSubDep1Idx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep2bycleardepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
		...
		Long argDefSchemaId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep2bydefschemaidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
