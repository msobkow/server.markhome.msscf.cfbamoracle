// Description: Java 11 Oracle Jdbc DbIO implementation for ClearSubDep3.
...
/*
 *	CFBamOracleClearSubDep3Table Oracle Jdbc DbIO implementation
 *	for ClearSubDep3.
 */
	...
	implements ICFBamClearSubDep3Table
{
		...
		long RelationId )
	{
		...
		List<CFBamClearSubDep3Buff> buffList = new LinkedList<CFBamClearSubDep3Buff>();
		try {
			stmtReadBuffByClearDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep3bycleardepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		Long DefSchemaId )
	{
		...
		List<CFBamClearSubDep3Buff> buffList = new LinkedList<CFBamClearSubDep3Buff>();
		try {
			stmtReadBuffByDefSchemaIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep3bydefschemaidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long ClearSubDep2Id )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByClearSubDep2Idx = null;
		List<CFBamClearSubDep3Buff> buffList = new LinkedList<CFBamClearSubDep3Buff>();
		try {
			stmtReadBuffByClearSubDep2Idx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_clrsubdep3byclearsubdep2idx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByClearSubDep2Idx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByClearSubDep2Idx.execute();
			resultSet = (ResultSet)stmtReadBuffByClearSubDep2Idx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamClearSubDep3Buff buff = unpackClearSubDep3ResultSetToBuff( resultSet );
		...
		long argClearSubDep2Id )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep3byclearsubdep2idx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByClearSubDep2Idx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep3bycleardepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
		...
		Long argDefSchemaId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_clrsubdep3bydefschemaidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
