// Description: Java 11 Oracle Jdbc DbIO implementation for SubProject.
...
/*
 *	CFBamOracleSubProjectTable Oracle Jdbc DbIO implementation
 *	for SubProject.
 */
	...
	implements ICFBamSubProjectTable
{
	...
	public String getSqlSelectSubProjectDistinctClassCode() {
		if( S_sqlSelectSubProjectDistinctClassCode == null ) {
			S_sqlSelectSubProjectDistinctClassCode =
				...
				+	"FROM " + schema.getLowerDbSchemaName() + ".sprjdef a105 ";
		}
		...
		long TopProjectId )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByTopProjectIdx = null;
		List<CFIntSubProjectBuff> buffList = new LinkedList<CFIntSubProjectBuff>();
		try {
			stmtReadBuffByTopProjectIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_sprjdefbytopprojectidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByTopProjectIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByTopProjectIdx.execute();
			resultSet = (ResultSet)stmtReadBuffByTopProjectIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFIntSubProjectBuff buff = unpackSubProjectResultSetToBuff( resultSet );
		...
		long argTopProjectId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_sprjdefbytopprojectidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByTopProjectIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
