// Description: Java 11 Oracle Jdbc DbIO implementation for SchemaRef.
...
/*
 *	CFBamOracleSchemaRefTable Oracle Jdbc DbIO implementation
 *	for SchemaRef.
 */
	...
	implements ICFBamSchemaRefTable
{
		...
		Long PrevId )
	{
		...
		List<CFBamSchemaRefBuff> buffList = new LinkedList<CFBamSchemaRefBuff>();
		try {
			stmtReadBuffByPrevIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_schema_refbyprevidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		Long NextId )
	{
		...
		List<CFBamSchemaRefBuff> buffList = new LinkedList<CFBamSchemaRefBuff>();
		try {
			stmtReadBuffByNextIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_schema_refbynextidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		Long argPrevId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_schema_refbyprevidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
		...
		Long argNextId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_schema_refbynextidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
		...
		int revision )
	{
		...
		try {
			stmtMoveUp = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".mvup_schema_ref( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", ? ); end;" );
			int argIdx = 1;
		...
		int revision )
	{
		...
		try {
			stmtMoveDown = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".mvdn_schema_ref( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", ? ); end;" );
			int argIdx = 1;
