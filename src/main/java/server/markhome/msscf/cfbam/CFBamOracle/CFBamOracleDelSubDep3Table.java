// Description: Java 11 Oracle Jdbc DbIO implementation for DelSubDep3.
...
/*
 *	CFBamOracleDelSubDep3Table Oracle Jdbc DbIO implementation
 *	for DelSubDep3.
 */
	...
	implements ICFBamDelSubDep3Table
{
		...
		long RelationId )
	{
		...
		List<CFBamDelSubDep3Buff> buffList = new LinkedList<CFBamDelSubDep3Buff>();
		try {
			stmtReadBuffByDelDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_delsubdep3bydeldepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long DelSubDep2Id )
	{
		...
		ResultSet resultSet = null;
		Connection cnx = schema.getCnx();
		CallableStatement stmtReadBuffByDelSubDep2Idx = null;
		List<CFBamDelSubDep3Buff> buffList = new LinkedList<CFBamDelSubDep3Buff>();
		try {
			stmtReadBuffByDelSubDep2Idx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_delsubdep3bydelsubdep2idx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadBuffByDelSubDep2Idx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadBuffByDelSubDep2Idx.execute();
			resultSet = (ResultSet)stmtReadBuffByDelSubDep2Idx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						CFBamDelSubDep3Buff buff = unpackDelSubDep3ResultSetToBuff( resultSet );
		...
		long argDelSubDep2Id )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_delsubdep3bydelsubdep2idx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
				...
				int rowsUpdated = stmtDeleteByDelSubDep2Idx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_delsubdep3bydeldepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
