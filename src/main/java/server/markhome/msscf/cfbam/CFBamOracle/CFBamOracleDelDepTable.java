// Description: Java 11 Oracle Jdbc DbIO implementation for DelDep.
...
/*
 *	CFBamOracleDelDepTable Oracle Jdbc DbIO implementation
 *	for DelDep.
 */
	...
	implements ICFBamDelDepTable
{
		...
		long RelationId )
	{
		...
		String classCode;
		ResultSet resultSet = null;
		CallableStatement stmtReadClassCodeByDelDepIdx = null;
		try {
			Connection cnx = schema.getCnx();
			stmtReadClassCodeByDelDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_del_depccbydeldepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
			stmtReadClassCodeByDelDepIdx.registerOutParameter( argIdx++, OracleTypes.CURSOR );
			...
			stmtReadClassCodeByDelDepIdx.execute();
			resultSet = (ResultSet)stmtReadClassCodeByDelDepIdx.getObject( 1 );
			if( resultSet != null ) {
				try {
					while( resultSet.next() ) {
						classCode = resultSet.getString( 1 );
		...
		long RelationId )
	{
		...
		List<CFBamDelDepBuff> buffList = new LinkedList<CFBamDelDepBuff>();
		try {
			stmtReadBuffByDelDepIdx = cnx.prepareCall( "begin " + schema.getLowerDbSchemaName() + ".rd_del_depbydeldepidx( ?, ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end;" );
			int argIdx = 1;
		...
		long argRelationId )
	{
		...
		try {
				Connection cnx = schema.getCnx();
				String sql = "begin call " + schema.getLowerDbSchemaName() + ".dl_del_depbydeldepidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ); end";
